...
def index(request):
    return render(request,'pigit/home.html')
...
def repolist(request):
    ...
    return render(request, 'pigit/repository.html', {'total': total, 'name': user_name})

def logout_page(request):
    """This is for logout function to clear the session"""
    del request.session['username']
    return HttpResponseRedirect("/pigit/login/")

def repo_details(request, name):
    ...
    if request.method == "POST":
        ...
        contents = my_repo.get_contents("", ref="main")
    ...
    return render(request, "pigit/repo_details.html",
...
def data(request, name):
    ...
    return render(request, "pigit/create_branch.html", {"name": name, "branches": my_branches})
def create_branch(request, name):
    ...
    if request.method == "POST":
        ...
        return repo_details(request, name)

        # return render(request, "pigit/reposit_details.html",
        #               {"name": name, "repo": repo, "branches": repo.get_branches(), "files": files})
    else:
        return render(request, "pigit/create_branch.html", {"name": name})
...
def file_details(request, name):
    ...
    my_branches = my_branch(request, name)
    repo = open_git.get_user().get_repo(repo_name)
    return render(request, "pigit/file.html", {"name": name, "branches": my_branches})
...
def pull_req(request, name):
    ...
    username = open_git.get_user().login
    my_branches = my_branch(request, name)
    return render(request, "pigit/pull_request.html", {"name": name, "branches": my_branches})
...
def merging(request, name):
    ...
    my_branches = my_branch(request, name)
    return render(request, "pigit/merging.html", {"branches": my_branches, "name": name})
...
def del_branch(request, name):
    ...
    my_branches = my_branch(request, name)
    return render(request, "pigit/del_branch.html", {"branches": my_branches, "name": name})
